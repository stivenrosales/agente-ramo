...
const STORE = "https://wilsonstore.com.pe";
const OUTPUT = resolve(process.cwd(), "data", "wilson-catalog.json");
const BRAND_LABEL = "Wilson Store Perú";
...
function inferGenero(
  ...
  const hay = [...tags, vendor, title, productType].join(" ").toLowerCase();
  const tieneHombre = /\bhombre\b|\bmasculino\b|\bmascul/.test(hay);
  const tieneMujer = /\bmujer\b|\bfemenino\b|\bfemenina\b|\bfemenin/.test(hay);
...
function normalize(p: ShopifyProduct): CatalogProduct | null {
  ...
  return {
    ...
    genero: inferGenero(p.tags, p.vendor, p.title, p.product_type),
    tags: p.tags.filter((t) => !/^Ropa$|^Calzado$|^Hombre$|^Mujer$|^Unisex$|Deportes/i.test(t)),
    price_min: priceMin,
    price_max: priceMax,
...
async function main() {
  console.log(`📦 Descargando catálogo de ${BRAND_LABEL}...`);
  ...
  [...byType.entries()]
    .sort((a, b) => b[1] - a[1])
    .forEach(([t, n]) => console.log(`   ${n.toString().padStart(3)}  ${t || "(sin tipo)"}`));
}
